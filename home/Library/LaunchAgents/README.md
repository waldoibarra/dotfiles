...
By placing your own plist files here, the [restart-brew-service-with-custom-plist.sh](/scripts/restart-brew-service-with-custom-plist.sh)
script can use them instead of Homebrew's defaults.
...
The plist is used by the service when running `make install` via
[install.conf.yaml](/install.conf.yaml).

## Manual Setup

Use `make brew-restart-service` to restart a service with its custom plist:

```bash
make brew-restart-service FORMULA=<formula-name>
```

Example for Ollama:

```bash
make brew-restart-service FORMULA=ollama
```
